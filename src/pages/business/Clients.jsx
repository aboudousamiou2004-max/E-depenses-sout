...
import { Users, Plus, Trash2, Pencil, Phone } from "lucide-react";
...
const VIDE = { nom: "", telephone: "", forfaitNiveau: "simple", dateDebut: new Date().toISOString().slice(0, 10), estPartenaire: false, entreprise: "", note: "" };

// Clients — volet spécifique à MAXI GYM : qui est abonné, à quel forfait,
// pour suivre les renouvellements. Les paiements eux-mêmes se saisissent
// depuis Prestations (facturation), comme pour tous les autres secteurs —
// cet écran ne fait que le suivi des personnes, pas la facturation.
// N'affiche que les clients individuels — les entreprises partenaires sont
// dans le volet séparé « Clients partenaires » (ClientsPartenaires.jsx),
// même table, filtrée sur `estPartenaire`.
export default function Clients() {
  ...
  const filtres = useMemo(
    () => clients.filter((c) => !c.estPartenaire).filter((c) => !filtreForfait || c.forfaitNiveau === filtreForfait),
    [clients, filtreForfait]
  );
  const individuels = clients.filter((c) => !c.estPartenaire);
  const actifs = individuels.filter((c) => c.actif).length;

  function openCreate() { setModal({ id: null, data: { ...VIDE } }); setError(""); }
  function openEdit(c) {
    setModal({ id: c.id, data: { nom: c.nom, telephone: c.telephone, forfaitNiveau: c.forfaitNiveau, dateDebut: c.dateDebut, note: c.note } });
    setError("");
  }
  ...
  async function enregistrer() {
    ...
    setSaving(true);
    setError("");
    const res = modal.id ? await modifierClient(modal.id, modal.data) : await ajouterClient(config.secteurId, modal.data);
    setSaving(false);
  ...
  async function confirmerSuppression(motif) {
    ...
    await enregistrerMotifSuppression({ user, table: "gym_clients", label: c.nom, motif, secteurId: config.secteurId });
    return supprimerClient(c.id);
  }

  return (
    <div>
      <TopBarSimple title="Clients" subtitle={`${config.nom} : suivi des abonnés`} icon={Users} accent={config.color} />

      <div className="grid grid-cols-2 gap-4 mb-5">
        <StatTile icon={Users} label="Clients actifs" value={String(actifs)} tone={config.color} />
        <StatTile icon={Users} label="Total fiches" value={String(individuels.length)} tone="#8E8E93" />
      </div>

      <div className="flex flex-wrap items-center gap-2 mb-4">
        <Select className="!w-auto" value={filtreForfait} onChange={(e) => setFiltreForfait(e.target.value)}>
          <option value="">Tous les forfaits</option>
          {NIVEAUX_FORFAIT.map((n) => <option key={n.id} value={n.id}>{n.label}</option>)}
        </Select>
        <Button icon={Plus} onClick={openCreate} style={{ background: config.color }} className="ml-auto">Nouveau client</Button>
      </div>
      ...
      <GlassCard className="p-2 overflow-hidden" hover={false}>
        <table className="w-full min-w-[680px] border-collapse">
          ...
          <tbody>
            {filtres.length === 0 && <tr><td colSpan={5} className="text-center py-10 text-[13px] text-ink-soft italic">Aucun client.</td></tr>}
            {filtres.map((c) => (
              <tr key={c.id} className="text-[13px] hover:bg-white/50 transition-colors">
                ...
                <td className="px-3 py-2.5 text-ink-soft tabular">{c.dateDebut ? new Date(c.dateDebut).toLocaleDateString("fr-FR") : "—"}</td>
      ...
      >
        {modal && (
          <form onSubmit={(e) => { e.preventDefault(); enregistrer(); }}>
            ...
            <Field label="Nom complet"><TextInput value={modal.data.nom} onChange={(e) => setModal((m) => ({ ...m, data: { ...m.data, nom: e.target.value } }))} autoFocus /></Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Téléphone" hint="Optionnel"><TextInput value={modal.data.telephone} onChange={(e) => setModal((m) => ({ ...m, data: { ...m.data, telephone: e.target.value } }))} placeholder="ex : 90 00 00 00" /></Field>
              <Field label="Forfait">
